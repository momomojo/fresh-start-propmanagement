...
import { z } from 'zod';
import { updateProperty } from '../../lib/store/slices/propertySlice';
...
const EditPropertyModal: React.FC<EditPropertyModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await dispatch(updateProperty({ id: property.id, data: validated }));
      onClose();
    } catch (error) {
      if (error instanceof z.ZodError) {
        ...
        setErrors({ form: 'Failed to update property. Please try again.' });
      }
    } finally {
      setIsLoading(false);
    }
